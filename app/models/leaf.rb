...
class Leaf < ActiveRecord::Base
  ...
  validates_attachment_content_type :photo,
    content_type: VALID_ATTACHMENT_TYPES, if: proc { |p| p.errors.blank? }
  validates_attachment_size :photo,
    less_than: MAX_UPLOAD_SIZE,
    message: 'File too big! :{', if: proc { |p| p.errors.blank? }
  ...
  def validates_photo_or_post
    if content.blank? && photo_file_name.blank?
      errors.add(:leaf, 'must have text or a picture, why would ' \
      'you want to make a blank post? ;3')
    end
  end
  ...
  def transliterate_file_name
    ...
    @photo.instance_write(
      :file_name,
      "#{time}#{transliterate(filename)}.#{transliterate(extension)}")
  end
